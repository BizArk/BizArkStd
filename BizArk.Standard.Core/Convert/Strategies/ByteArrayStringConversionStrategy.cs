...
namespace BizArk.Standard.Core.Convert.Strategies
{
		...
		: IConvertStrategy
	{
		...
		public bool TryConvert(object value, Type to, out object convertedValue)
		{
			...
			if (from == typeof(byte[]))
			{
				...
				convertedValue = (Encoding ?? Application.DefaultEncoding).GetString(strBytes);
				return true;
			}
			...
			convertedValue = (Encoding ?? Application.DefaultEncoding).GetBytes(str);
			return true;
		}
