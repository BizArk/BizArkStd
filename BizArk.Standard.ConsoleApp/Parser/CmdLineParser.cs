...
namespace BizArk.Standard.ConsoleApp.Parser
{
	...
	internal class CmdLineParser<T> where T : new()
	{
		...
		public CmdLineParser(CmdLineOptions options = null)
		{
			if (options == null)
			{
				...
				var att = typeof(T).GetTypeInfo().GetCustomAttribute<CmdLineOptionsAttribute>(false);
				if (att != null)
		...
		private void FillCmdLineObjInfo(CmdLineParseResults<T> results)
		{
			...
			var att = this.GetType().GetTypeInfo().GetCustomAttribute<DescriptionAttribute>(true);
			if (att != null)
		...
		internal CmdLinePropertyBag GetCmdLineProperties(T obj)
		{
			...
			{
				var prop = GetCmdLineProperty(propInfo, obj);
				if (prop != null)
					props.Add(prop);
			}
		...
		private CmdLineProperty GetCmdLineProperty(PropertyInfo propInfo, T obj)
		{
			var prop = new CmdLineProperty(propInfo, obj);

			prop.Required = propInfo.GetCustomAttribute<RequiredAttribute>() != null;

			var claAtt = propInfo.GetCustomAttribute<CmdLineArgAttribute>();

			// Make sure aliases has a valid value so we don't have to check for nulls everywhere.
			prop.Aliases = claAtt?.Aliases ?? new string[] { };

			// If CmdLineArgAttribute.ShowInHelp is not set, show in help only if it is required.
			prop.ShowInHelp = claAtt?.ShowInHelp ?? prop.Required;

			// If CmdLineArgAttribute.ShowInUsage is not set, show in usage only if it is required.
			prop.ShowInUsage = claAtt?.ShowInUsage ?? prop.Required;
